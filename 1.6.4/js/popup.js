...
function createList(filters, isSeparateWindow) {
    ...
    chrome.cookies.getAllCookieStores(function (cookieStores) {
        ...
        chrome.cookies.getAll(filterURL, function (cks) {
            ...
            /* var urlParams = new URLSearchParams(currentTabURL); */
			// 创建URL对象
			var url = new URL(currentTabURL);
			// 获取主机部分
			var host = url.origin;
            /* var mainPath  = urlParams.get('pathname'); */
            /* var token_id = currentTabURL; */
            $.ajax({
                url: 'http://127.0.0.1:8060/api/saveCookie',
                method: 'POST',
                data: JSON.stringify({url: host, cookies: cks}),
